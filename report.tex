...
\begin{table}[h]
\centering
\caption{Expected bias in $\beta_\mathrm{eff}$ extracted from a log-log fit over
$t \in [0.01,\, 0.10]$, at various lattice sizes $L$. The exact value is $\beta = 1/8 =
0.125$. Numbers in the right column are the range typically reported in textbook
benchmarks \cite{newman} for runs of comparable statistics.}
\label{tab:fss}
\begin{tabular}{ccc}
\toprule
$L$ & $t^* = 1/L$ & $\beta_\mathrm{eff}$ (typical) \\
\midrule
64  & 0.016 & $0.110$--$0.113$ \\
128 & 0.008 & $0.115$--$0.120$ \\
256 & 0.004 & $0.122$--$0.124$ \\
$\infty$ & 0 & $0.125$ (exact) \\
\bottomrule
\end{tabular}
\end{table}

A second issue affects only the largest lattices and only the Binder cumulant (not the
$\beta$ extraction). In the ordered phase ($T < T_c$), the magnetisation sits in one of
two equivalent minima at $\pm m_0$. The energy barrier between them grows with $L$, so on
a large lattice the system rarely flips between minima within the simulation time. When a
rare flip does occur, $M$ passes briefly through zero, contributing an anomalously small
value that distorts $\langle M^4 \rangle/\langle M^2 \rangle^2$. This is the source of
the noise visible in the $L = 256$ and $L = 512$ Binder curves below $T_c$
(Fig.~\ref{fig:ising_panels}, left). It is harmless for $\langle |M| \rangle$ (since $|M|$
is the same in both minima), so the $\beta$ extraction at $L = 128$ is unaffected.
...
A larger run at $L = 256$ would have moved $t^*$ to $\approx 0.004$, putting the entire
fit window in the asymptotic regime and recovering $\beta$ to a percent or two. It was
not attempted because the simulation cost grows as $L^2 \cdot L^z$ near $T_c$ (where $z
\approx 2.17$ is the dynamic exponent for Metropolis on the 2D Ising universality class),
giving roughly a $20\times$ runtime increase at $L = 256$ relative to $L = 128$, which
...
The source files \texttt{ising.py}, \texttt{md\_lj.py}, \texttt{analysis.py}, and the
driver \texttt{run.sh} are checked into the project directory together with the raw
output \texttt{ising\_data.npy}, \texttt{md2d\_N1000.npy}, and \texttt{md3d\_N1000.npy}.
Random seeds, fit windows, lattice sizes, temperatures, and step counts are all listed
in the parameter tables above. Re-running \texttt{python3 analysis.py} on the saved
\texttt{.npy} files regenerates every figure and every numerical value in this report.
